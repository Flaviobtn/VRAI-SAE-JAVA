...
/*
    public Vendeur getVendeur(String idVendeur)throws SQLException{
		...
		try{
			while(rs.next()){
                ...
                return new Vendeur(idV, nom,ville, magBD.getMagasin(mag));
			}
		}
    ...
    public void seconnecterVendeur(String idVendeur, String mdp) throws SQLException {
        String req = "SELECT * FROM VENDEUR WHERE idVendeur = ? AND mdp = ?";
        this.st = laConnexion.prepareStatement(req);
        st.setString(1, idVendeur);
        st.setString(2, mdp);
        ResultSet rs = st.executeQuery();
        if (rs.next()) {
            System.out.println("Connexion réussie pour le vendeur avec ID: " + idVendeur);
        } else {
            System.out.println("Échec de la connexion pour le vendeur avec ID: " + idVendeur);
        }
        rs.close();
    }
    ...
    public void insertVendeur(Vendeur vendeur){
		try {
            ...
            st.setString(3, vendeur.getMagasin().getIdmag());
		} catch (SQLException e) {
			System.err.println(e.getMessage());
		}
    }
