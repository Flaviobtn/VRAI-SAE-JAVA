...
public class ClientBD{
	...
	ClientBD(Connection laConnexion){
	...
	public Client getClient(int idcli)throws SQLException{
		...
		try{
			while(rs.next()){
				...
				return new Client(idClient, nomCli,prenomCli, addresseCli, codePostalCli, villeCli);
			}
			rs.close();
		}
	...
	public boolean verifierIdentifiant(String ident) throws SQLException {
		String req = "Select * FROM CLIENT WHERE ident = ?";
		this.st = laConnexion.prepareStatement(req);
		this.st.setString(1, ident);
		ResultSet rs = st.executeQuery(req);
		try {
			if(rs.next()) {
	...
	public void seconnecterClient(String ident, String mdp) throws SQLException {
		String req = "Select * FROM CLIENT WHERE ident = ? AND mdp = ?";
		this.st = laConnexion.prepareStatement(req);
		this.st.setString(1, ident);
		...
		try {
			if(rs.next()) {
				System.out.println("Connexion réussie pour le client: " + rs.getString("nomcli"));
			} else {
	...
	public void insererClient(Client client){
		try {
			...
			st.setString(5, String.valueOf((client.getCodePostal())));
			st.setString(6, client.getVille());
