...
public class VendeurBD {
    ...
    public Vendeur getVendeur(String idcli)throws SQLException{
		String req = "Select * FROM VENDEUR WHERE icli = ?";
		this.st = laConnexion.prepareStatement(req);
		st.setInt(1, idcli);
		ResultSet rs = st.executeUpdate(req);
		try{
			while(rs.next()){
				String idClient = rs.getInt("idcli");
				nom varchar(50),
                prenom varchar(50),
                idmag VARCHAR(42) NOT NULL
				Auteur client = new Auteur(nomCli, prenomCli, numCli, addresseCli, codePostalCli, villeCli);
			}
		}
